...
public class batmove : MonoBehaviour {
	...
	void Update () {
		...
		if (distance < 5) {
			if (transform.position.x - hero.position.x > 0 && transform.position.y - hero.position.y > 0)
				rigidbody2D.velocity = new Vector2 (-1,-1);
			else if (transform.position.x - hero.position.x > 0 && transform.position.y - hero.position.y <= 0)
				rigidbody2D.velocity = new Vector2 (-1,1);
			else if (transform.position.x - hero.position.x <= 0 && transform.position.y - hero.position.y <= 0)
				rigidbody2D.velocity = new Vector2 (1,1);
			else rigidbody2D.velocity = new Vector2 (1,-1);
